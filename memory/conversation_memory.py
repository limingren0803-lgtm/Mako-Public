...
class MemoryManager:
    ...
    async def update_profile(self, user_id: str, conv_id: str) -> None:
        ...
        user_messages = [m for m in messages[-10:] if m.role == MsgRole.USER]
        ...
        career_schema = json.dumps(
            CareerProfile.model_json_schema(),
            ensure_ascii=False,
)

        prompt = (
            """从以下用户消息中提炼用户画像，并严格返回一个 JSON 对象。

            要求：
            1. 只提取用户本人明确表达的信息，不得使用助手的推荐、推断或评价作为用户事实。
            2. 不确定的信息使用 unconfirmed；用户明确陈述的信息使用 confirmed。
            3. 不得把推测写成事实。
            4. 不得根据职业常识自动补充用户没有提供的信息。
            5. 所有 CareerProfile 字段必须严格符合下方 Pydantic JSON Schema。
            6. location_preferences 必须使用 location，不得使用 city。
            7. target_roles 只记录用户本人明确表达的目标岗位或方向，不记录助手推荐的岗位。
            8. strengths、gaps 和 risks 只有在用户明确陈述，或能够直接由用户事实无争议地表达时才能记录；不得自行制造风险或能力缺口。
            9. source 字段统一填写 "user"。
            10. 只返回紧凑 JSON，不要解释、不要 Markdown、不要重复用户消息内容。
        ...
        try:
            ...
            if s == -1 or e == -1 or e < s:
                raise ValueError("LLM 未返回合法 JSON 对象")
            ...
            existing_career_profile = existing_profile.get("career_profile") or {}
            
            career_profile_raw = profile_data.get("career_profile") or {}
            
            profile_data["career_profile"] = self._merge_career_profile(
                existing_career_profile,
                career_profile_raw,
            )
    ...
    def _merge_career_profile(
        ...
        规则：
        1. incoming 为空时保留 existing。
        2. 列表字段只追加新内容，不删除旧内容。
        3. 普通字段仅在 incoming 有有效值时更新。
        4. 合并后再次通过 CareerProfile 校验。
        """

        existing_model = CareerProfile.model_validate(existing or {})
        incoming_model = CareerProfile.model_validate(incoming or {})

        existing_data = existing_model.model_dump(mode="json")
        incoming_data = incoming_model.model_dump(mode="json")
        ...
        for field_name in CareerProfile.model_fields:
            ...
            if field_name in list_fields:
                ...
                if field_name == "location_preferences":
                    ...
                    for item in old_list + new_list:
                        ...
                        previous = locations.get(location, {})
                    ...
                    merged[field_name] = list(locations.values())
                    continue
        ...
        return CareerProfile.model_validate(merged).model_dump(
         mode="json",
         by_alias=True,
        )
    ...
    async def _get_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            latest_index = max(
                range(len(documents)),
                key=lambda i: (metadatas[i] or {}).get("ts", ""),
            )
