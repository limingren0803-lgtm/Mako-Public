...
DEFAULT_INTENT_CASES: List[IntentTestCase] = [
    # 通用意图：保留基础 Agent / Intent 能力测试
    IntentTestCase("这个系统支持哪些功能？", "query"),
    IntentTestCase("请帮我整理一下这段信息", "request"),
    IntentTestCase("Swagger 接口一直报 500 错误", "technical"),
...
DEFAULT_DIALOG_CASES: List[Dict[str, Any]] = [
    {"question": "根据你已经保存的职业画像，我目前的求职方向是什么？"},
    {
        "question": (
            "帮我分析下面这个产品运营岗位 JD 的核心职责和能力要求："
            "负责用户增长活动策划与落地，跟踪新增、活跃和留存指标；"
            "结合用户反馈和数据分析提出产品优化建议，并协调产品、设计、研发推进上线；"
            "要求本科及以上学历，熟练使用 Excel，具备基础 SQL 能力、项目推进能力和良好沟通能力；"
            "有互联网运营实习或校园项目经验者优先。"
        )
    },
    {"question": "根据我的背景和这个 JD，分析我的岗位匹配度"},
    {"question": "请根据目标岗位优化我的实习经历描述"},
