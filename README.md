...
Mako 提供面向求职用户的 Web 工作台，将六项 Career Skill 分为背景诊断、方向匹配、JD 分析、简历优化、面试准备和行动规划。每个板块采用一致的四步流程：选择功能、提供材料、确认输入、查看结果，同时提供适合当前任务的输入提示、准备清单、结果说明和问题处理建议。

工作台支持直接输入问题，也可以读取 UTF-8 编码的 `.txt` 或 `.md` 材料，用于提交简历文本、JD 或经历说明。文件文本随本次请求处理，不会作为知识文档导入。当前页面未接入 PDF、DOCX、用户账户或管理接口。

方向匹配板块支持从本地已审核职位中选择，也支持用户自行提供完整 JD。系统职位可以展示经过审核的官方 JD 职责或要求，由用户选择本次需要核对的条目和关键词；自行提供的 JD 通过现有 `/chat` 流程完成本次分析，可选链接只作为来源标注，页面不会主动访问。材料和匹配结果仅用于当前请求，不写入 CareerProfile、Redis、ChromaDB、职位库或长期证据库。

问题、经历、文件、JD、职位或岗位选项发生变化后，页面会撤销此前的材料确认。分析结果支持复制、下载 TXT 和重新开始；服务暂时不可用时，当前输入会保留，用户可以检查后直接重试。
