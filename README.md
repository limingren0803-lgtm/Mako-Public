...
默认目录只收录能够确认企业归属的招聘网站，当前覆盖 15 家企业：
...
每个来源记录官方招聘域名、入口、行业、招聘渠道、支持等级和核验日期。`GET /jobs/sources` 提供只读目录，并支持按行业和支持等级筛选。目录中的企业可以通过受保护接口导入人工核验的结构化 JD；官方页面公开提供 Schema.org `JobPosting` 时，也可以按指定 URL 导入。
