...
class Skill:
    ...
    def matches_intent(self, intent: Optional[str], agent_type: Optional[str] = None) -> bool:
        """按已识别 Intent 精确匹配，避免多个 Career Skill 同时注入。"""
        if not self.enabled:
            return False
        if self.agents and agent_type and agent_type.lower() not in self.agents:
            return False
        return bool(intent and self.intent and self.intent.lower() == intent.lower())

    def to_prompt_block(self) -> str:
        """格式化为可直接拼入 system prompt 的文本块。"""
...
class SkillManager:
    ...
    def __init__(self, root_dir: str, max_prompt_chars: int = 18000):
    ...
    def prompt_for(
        ...
        intent_matches = [
            ...
            if skill.matches_intent(intent, agent_type)
        ]
        has_intent_scoped_skills = any(
            skill.intent
            and skill.enabled
            and (not skill.agents or not agent_type or agent_type.lower() in skill.agents)
            for skill in self._skills
        )

        if intent_matches:
            candidates = intent_matches
        elif intent and has_intent_scoped_skills:
            # CareerAgent 已有 Intent 绑定时，未知 Intent 不得串入其他 Skill。
            candidates = []
        else:
            # General/Technical 等未绑定 Intent 的 Agent 保留关键词回退。
            candidates = self._skills

        for skill in candidates:
            if not intent_matches and not skill.matches(message, agent_type):
                continue
            matched_keywords = [
    ...
    def _discover_files(self, root_dir: Path) -> Iterable[Path]:
        ...
        for path in sorted(root_dir.rglob("*")):
            ...
            if (
                ...
                or path.name.upper() == "README.MD"
                or "_backup" in lower_name
    ...
    def _load_text(self, path: Path) -> Optional[Skill]:
        ...
        return Skill(
            ...
            intent=str(meta.get("intent") or "").strip() or None,
        )
