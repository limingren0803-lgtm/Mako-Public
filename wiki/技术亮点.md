...
本文档说明 Mako 当前实现中的七大核心技术亮点。每个亮点都对应真实代码模块，并能通过 API 或 Docker 环境观察到效果。

特别强调：端到端评测是 Mako 当前已经实现的核心能力之一，不是后续规划。系统通过 `POST /eval/run` 支持内置用例和自定义用例评测，覆盖意图识别 Accuracy/Macro-F1、真实 Agent 回复质量、LLM-as-Judge 四维评分、回归检测和优化建议生成。也就是说，Mako 的技术亮点不仅包括“会回答”，还包括“能自动评估回答质量并发现退化”。
...
**核心问题**：职业决策系统需要判断用户是在做背景诊断、岗位匹配、JD 分析、简历优化、面试准备还是行动规划。单靠关键词容易误判，单靠 LLM 成本和延迟较高，因此 Mako 使用三路融合。
...
Mako 中 ChromaDB 不是单一用途，而是同时承担 RAG 知识库和长期记忆。
