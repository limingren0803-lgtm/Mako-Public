...
本文档展示 Mako 的核心架构图。图片均已生成到 `wiki/assets/architecture/`，可直接在 wiki、README 或项目展示材料中引用。

当前架构重点体现：Docker Compose 一键部署、多 Agent 编排、RAG 知识库、三级记忆、动态 Skills、监控和端到端评测。

## 1. 总体架构

![Mako 总体架构](assets/architecture/01-overall-architecture.svg)

说明：

- 用户请求先进入 Nginx，再转发到 Mako 主服务。
...
![Mako /chat 主链路架构](assets/architecture/02-chat-flow.svg)
...
![Mako 多 Agent 与 Skills 注入关系](assets/architecture/03-agent-skills.svg)
...
![Mako 数据与存储架构](assets/architecture/04-data-storage.svg)
...
![Mako 监控与评测闭环](assets/architecture/05-monitor-eval.svg)
...
![Mako Docker Compose 一键部署结构](assets/architecture/06-deployment.svg)
