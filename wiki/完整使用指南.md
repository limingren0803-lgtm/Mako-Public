...
本文档说明 Mako 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

重要说明：Mako 当前已经支持可直接调用的端到端评测能力。启动 HTTP 服务后，可以通过 Swagger 或 `curl` 调用 `POST /eval/run`，自动评测意图识别准确率、端到端 Agent 回复质量、LLM-as-Judge 四维评分、回归检测和优化建议。该能力不是文档设计稿，而是由 `evaluation/evaluator.py` 和 `api/main.py` 中的 `/eval/run` 接口真实实现。

Mako 是一个 AI Career Intelligence System，核心链路为：
...
ECHOMIND_SKILLS_MAX_PROMPT_CHARS=18000
```

### 2.3 全栈部署和 run 开发模式的区别

Mako 常用两种 Docker 启动方式：`docker compose up` 全栈部署，以及 `docker run` 开发模式。两者最大的区别是：**全栈部署会同时启动应用和依赖服务；run 开发模式通常只手动运行一个应用容器，依赖服务需要提前启动**。

| 对比项 | Docker Compose 全栈部署 | Docker run 开发模式 |
|--------|--------------------------|----------------------|
| 启动命令 | `docker compose up -d --build` | `docker run ... echomind ...` |
| 启动内容 | Mako、Redis、ChromaDB、Prometheus、Nginx | 只启动你指定的单个容器 |
...
看到 Mako 启动日志并且健康检查通过后，服务可用。

启动后的端口：

| 服务 | 容器名 | 宿主机端口 | 容器内端口 | 用途 |
|------|--------|------------|------------|------|
| Mako API | `echomind-app` | `8000` | `8000` | 主 API 服务 |
...
Mako 基于 FastAPI 构建，启动 HTTP 服务后可以直接在浏览器访问 Swagger UI 调用接口。
...
Mako 的知识库由 `mcp/knowledge_base.py` 管理，底层使用 ChromaDB collection：
...
Mako 使用了三个 ChromaDB collection：
...
# 6. 检索
curl -X POST "http://localhost:8000/search?query=Mako如何接入API&top_k=3"
